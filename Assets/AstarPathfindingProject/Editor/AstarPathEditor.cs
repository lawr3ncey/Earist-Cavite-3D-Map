	...
	public class AstarPathEditor : Editor {
			...
			if (script.prioritizeGraphs) {
				...
				if (moveUp || moveDown) {
					...
					if (next >= 0 && next < script.graphs.Length) {
                        ...
                        (graphEditors[index], graphEditors[next]) = (graphEditors[next], graphEditors[index]);
                    }
                    CheckGraphEditors();
					Repaint();
				}
		...
		void DrawGraphInfoArea (GraphEditor graphEditor) {
			...
			if (graphEditor.infoFadeArea.BeginFade()) {
                ...
                graphNodeCounts ??= new Dictionary<NavGraph, KeyValuePair<float, KeyValuePair<int, int>>>();

                if (!graphNodeCounts.TryGetValue(graphEditor.target, out KeyValuePair<float, KeyValuePair<int, int>> pair) || (Time.realtimeSinceStartup-pair.Key) > 2) {
			...
			if (serializationSettingsArea.BeginFade()) {
				...
				if (GUILayout.Button("Save to file")) {
					...
					if (path != "") {
                        ...
                        var bytes = SerializeGraphs(serializationSettings, out uint checksum);
		...
		void SerializeIfDataChanged () {
            ...
            byte[] bytes = SerializeGraphs(out uint checksum);
		...
		public byte[] SerializeGraphs (Pathfinding.Serialization.SerializeSettings settings, out uint checksum) {
			...
			bytes = script.data.SerializeGraphs(settings, out uint tmpChecksum);
